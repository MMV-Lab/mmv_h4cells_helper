...
savedir = Path(path_raw_images.parent, 'nuclei')
model_path = 'models/nuclei'

chan = 3 # 0 if grayscale, 1/2/3 if RGB, so 1 for red, 2 for green, 3 for blue
chan2 = 0
flow_threshold = 0.4
...
for fn in tqdm(filenames):
    ...
    out_path = savedir / fn.name #.replace('.tif','_nuclei.tif' ,-1)
    OmeTiffWriter.save(masks[0], out_path, dim_order="YX")                                  
    end = time.time()
    print(f"Processing {fn.name} took {int(end-start)} seconds (nuclei)")